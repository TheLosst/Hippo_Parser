    ...
    public static void main(String[] args) throws IOException {
        ...
        try (FileReader reader = new FileReader("C:\\Users\\TheLosst\\IdeaProjects\\Parser_Hippo\\src\\com\\company\\The_Losst\\File.txt")) {
            ...
            while ((c = reader.read()) != -1) {
                all_text.append((char) c);
            }
            //System.out.println(all_text);
        }
        all_text = new StringBuilder(all_text.toString().replaceAll("\\n", ""));
        all_text = new StringBuilder(all_text.toString().replaceAll("\\r", ""));
        ...
        while (global < amount_of_horses) {
            String NOMER = "";
            StringBuilder klichka = new StringBuilder();
            StringBuilder mast = new StringBuilder();
            StringBuilder naezdnik = new StringBuilder();
            boolean number_try = false, klichka_try = false, mast_try = false;
            while (!(number_try)) {
                if ((all_text.charAt(count) == '.')) {
                    NOMER = all_text.charAt(count - 2) + String.valueOf(all_text.charAt(count - 1)) + all_text.charAt(count);
                    number_try = true;
            ...
            while (!(klichka_try)) {
                if ((String.valueOf(all_text.charAt(count + 1)).equals("-"))) {
                    for (int i = count_before; i < count + 1; i++) {
                        klichka.append(all_text.charAt(i));
            ...
            for (int i = count_before; i < resetID; i++) {
                naezdnik.append(all_text.charAt(i));
            }
            count = resetID + 15;
            naezdnik.append(".");
            ...
            global++;
            System.out.println("--------------------------------------------------------------------------");
            Hippo_Parser part = new Hippo_Parser(NOMER, klichka.toString(), mast.toString(), naezdnik.toString());
